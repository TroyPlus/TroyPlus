...
namespace Troy.Web.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        //---- check unique key-------   
        public JsonResult CheckForDuplication([Bind(Prefix = "Employee.Emp_No")]int Emp_No, [Bind(Prefix = "Employee.Emp_Id")]int? Emp_Id)
        {
            try
            {
                if (Emp_Id != null)
                {
                    if (!(employeeRepository.CheckDuplicateNameWithId(Convert.ToInt32(Emp_Id), Emp_No)))
                    {
        ...
        private void XMLGenerate_SAPInsert(EmployeeViewModels model)
        {
            try
            {
                ...
                addEmp.personal.Gender = Convert.ToString(model.Employee.Gender_ID);
        ...
        private void XMLGenerate_SAPUpdate(EmployeeViewModels model)
        {
            try
            {
                ...
                modifyEmp.personal.Gender = Convert.ToString(model.Employee.Gender_ID);
                        ...
                        if (ds != null)
                        {
                            ...
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                for (int j = 0; j < ds.Tables[0].Rows.Count; j++)
                                {
                                    ...
                                    {
                                        mItem.Gender_ID = 1;// Convert.ToInt32(ds.Tables[0].Rows[j]["Gender"].ToString());
                                    }
                                    else
                                    {
        ...
        public PartialViewResult _EditPartial(int id)
        {
            try
            {
                ...
                //GetManager();
                var qList = employeeRepository.GetAllEmployee();
                model.EmployeeList = qList;
